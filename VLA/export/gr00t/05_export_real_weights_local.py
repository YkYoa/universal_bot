#!/usr/bin/env python3
"""
Ban LOCAL (chay tren laptop, khong can huyhoang-4090/IQ-9075) cua
05_export_real_weights.py - load TRONG SO THAT (khong random-init) cho 4
layer dau cua DiT tu checkpoint GR00T-N1.7-3B da tai truc tiep tu
HuggingFace ve laptop, export ONNX, luu input+output tham chieu (fp32) de
so sanh voi output sau quantize INT8.

Khac ban goc: dung checkpoint local (khong qua huyhoang-4090), va co the
tan dung GPU CUDA that cua laptop (RTX 4050) cho buoc forward tham chieu
(khong anh huong ket qua export ONNX - van trace tren CPU nhu ban goc de
giu tinh portable).
"""
import json
import os
...
CKPT = "/home/hans/universal_bot/VLA/weights_local/gr00t_n1.7_3b"
OUT = "/home/hans/universal_bot/VLA/export/gr00t/dit_real_weights_local"
