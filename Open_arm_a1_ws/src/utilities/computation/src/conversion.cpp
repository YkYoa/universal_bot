...
geometry_msgs::msg::Pose vecToPoseMsgs(const std::vector<double>& pose)
{
    ...
    if (pose.size() < 7) {
        std::cerr << "vecToPoseMsgs: input vector size error" << std::endl;
        return pose_msg;
    }
...
tf2::Transform eigenToTfTransform(const Eigen::Matrix4d& eigen_mat)
{
    ...
    tf2::Matrix3x3 tf2_rotation(eigen_mat(0, 0), eigen_mat(0, 1), eigen_mat(0, 2),
...
tf2::Vector3 StringToTfvec3(const std::string& input_str)
{
    ...
    std::vector<double> vals = StringToDoubleVec(clean);
    if (vals.size() >= 3) {
        return tf2::Vector3(vals[0], vals[1], vals[2]);
    }
...
tf2::Quaternion StringToTfQuat(const std::string& input_str)
{
    ...
    std::vector<double> vals = StringToDoubleVec(clean);
    if (vals.size() >= 4) {
        return tf2::Quaternion(vals[0], vals[1], vals[2], vals[3]);
    }
