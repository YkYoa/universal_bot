...
Kinematics::Kinematics(const std::string& urdf_xml)
{
  ...
  if (!model.initString(urdf_xml)) {
    throw std::runtime_error("hand_kinematic: failed to parse robot_description URDF");
  }
...
{
  auto sol = boundedLeastSquares(residual_fn, x0, lower, upper, 1e-10, 1e-10, 200);
  if (sol.cost > 1e-9 && anyNonzero(x0)) {
    auto sol_rest =
      boundedLeastSquares(residual_fn, Eigen::VectorXd::Zero(x0.size()), lower, upper, 1e-10, 1e-10, 200);
...
Eigen::VectorXd HandKinematicNode::residualServo(
...
Eigen::VectorXd HandKinematicNode::residualKnuckle(
  const FingerInfo& f, const Eigen::VectorXd& x, double q1, double q_prox) const
{
...
: rclcpp::Node("hand_kinematic_node", options)
{
  ...
  command_space_ = declare_parameter<std::string>("command_space", "knuckle");
  ...
  if (urdf_xml.empty()) {
    throw std::runtime_error("robot_description parameter is empty; can't build kinematics");
  }
  if (command_space_ != "servo" && command_space_ != "knuckle") {
    throw std::runtime_error("command_space must be 'servo' or 'knuckle', got '" + command_space_ + "'");
  }
  ...
  RCLCPP_INFO(
    get_logger(), "command_space=%s, link_prefix='%s'; discovered %zu fingers", command_space_.c_str(),
    link_prefix.c_str(), fingers_.size());
  for (std::size_t i = 0; i < fingers_.size(); ++i) {
    const FingerInfo& f = fingers_[i];
    RCLCPP_INFO(
      get_logger(), "  finger %zu: %sj1%zu=yaw(%s) %sj2%zu=flex(%s) gimbal=%s servos=%s,%s", i + 1,
      alias_prefix.c_str(), i + 1, f.q1_joint.c_str(), alias_prefix.c_str(), i + 1, f.prox_joint.c_str(),
      f.gimbal.c_str(), f.servo_data[0].actuator_joint.c_str(), f.servo_data[1].actuator_joint.c_str());
  }
  ...
  // each node instance only reports its own side.
...
std::pair<LeastSquaresResult, std::map<std::string, double>> HandKinematicNode::solveFingerServo(
...
std::pair<LeastSquaresResult, std::map<std::string, double>> HandKinematicNode::solveFingerKnuckle(
  FingerInfo& f, const std::map<std::string, double>& raw)
{
  auto get_or_0 = [&](const std::string& k) {
    auto it = raw.find(k);
  ...
  Eigen::VectorXd lower(6), upper(6);
  lower << f.servo_data[0].lower, f.servo_data[1].lower, -1.2, -1.2, -0.02, -3.2;
  upper << f.servo_data[0].upper, f.servo_data[1].upper, 1.2, 1.2, 0.02, 3.2;
...
void HandKinematicNode::solveRodChains(
  FingerInfo& f, const std::map<std::string, double>& raw, std::map<std::string, double>& solved)
{
  // See FingerInfo::rod_solved_generation - replaces python's `is` identity
  // check on the cached solved-result dict.
...
void HandKinematicNode::onJointStates(const JointStateMsg& msg)
{
  ...
  for (std::size_t i = 0; i < msg.name.size(); ++i) {
    if (alias_of_.count(msg.name[i])) {
      continue;
    }
    by_name[msg.name[i]] = out_names.size();
    out_names.push_back(msg.name[i]);
    out_pos.push_back(msg.position[i]);
  }
  ...
  for (auto& f : fingers_) {
    ...
    if (sol.cost > 1e-6) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 2000, "finger %s: command out of mechanism reach (residual %.2e)",
        f.gimbal.c_str(), sol.cost);
    }

    for (const auto& [jn, v] : solved) {
      auto it = by_name.find(jn);
      if (it != by_name.end()) {
        out_pos[it->second] = v;
      } else {
        by_name[jn] = out_names.size();
        out_names.push_back(jn);
        out_pos.push_back(v);
      }
    }
  }
...
void HandKinematicNode::republishStates()
{
  ...
  for (const auto& [alias, val] : alias_state_) {
