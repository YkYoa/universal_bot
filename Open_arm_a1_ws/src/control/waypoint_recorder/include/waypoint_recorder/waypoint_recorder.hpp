...
/// Converts "myWaypointName" / "my_waypoint_name" style input to
/// "MyWaypointName" for the "<arm><PascalSection><N>Angle" key convention.
...
{
public:
  /// Subscribes to /joint_states; `timeout_sec` bounds each capture in
  /// recordOne()/recordBoth()/recordLoop().
